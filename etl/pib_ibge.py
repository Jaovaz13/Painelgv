    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data and len(data) > 0:
                ...
                for item in data:
                    if item.get("localidade", {}).get("id") == str(COD_IBGE):
...
def salvar_pib_no_banco(session: Session, dados: List[Dict], variavel: str, nome_indicador: str) -> None:
    ...
    logger.info("Salvos %s registros de %s", len(dados), nome_indicador)
